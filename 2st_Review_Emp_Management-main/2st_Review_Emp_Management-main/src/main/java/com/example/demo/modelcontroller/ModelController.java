	...
	public void deletid(@PathVariable int id)
{
	...
	//paging
		...
		//select by query
		@GetMapping("selectjpql/{empid}")
		List<ModelSeat>jpql(@PathVariable int empid){
			return me.getinfos(empid);
		}
		//delete by query
	 	@DeleteMapping("deletejpql/{empid}")

	 	public String DeleteDetailsByQuery(@PathVariable int empid) {
	 		return me.DeleteByQuery(empid)+"Deleted Successfully";
	 	}
